...
```{r}
Link <- "http://www.stamats.de/wp-content/uploads/2018/04/ITSDaten.csv"
...
```{r, eval=FALSE}
install.packages(c("DescTools", "ggplot2", "MKdescr"))
```
...
```{r}
prop.table(table(ITSDaten$Geschlecht, ITSDaten$OP), margin = 1)
```


```{r}
round(100*prop.table(table(ITSDaten$Geschlecht, ITSDaten$OP), margin = 1), 1)
```
...
```{r}
tapply(ITSDaten$OP, ITSDaten$Geschlecht, table)
```

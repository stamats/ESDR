...
perm.t.test(Temperatur ~ Geschlecht, data = ITSDaten[-398,])
...
wilcox_test(Temperatur ~ Geschlecht, data = ITSDaten, conf.int = TRUE,
...
```{r}
kruskal_test(Temperatur ~ Ergebnis, data = ITSDaten, 
             distribution = "approximate")
```
...
```{r}
## paarweise Welch t-Tests ohne Patient 398
pairwise.ext.t.test(ITSDaten$Temperatur[-398], ITSDaten$Ergebnis[-398],
                pool.sd = FALSE)
## paarweise Wilcoxon-Mann-Whitney U-Tests
pairwise.wilcox.exact(ITSDaten$Temperatur, ITSDaten$Ergebnis)
```
...
```{r}
## Paarweise gepaarte t-Tests
pairwise.ext.t.test(SE.lang$Score, SE.lang$Zeitpunkt, paired = TRUE)
## Paarweise Wilcoxon Vorzeichenrangtests
pairwise.wilcox.exact(SE.lang$Score, SE.lang$Zeitpunkt, paired = TRUE)
```
...
## Kendall
KendallTauB(ITSDaten$Temperatur, ITSDaten$Herzfrequenz, conf.level = 0.95)
```
...
cor.test(rank(ITSDaten$Temperatur), rank(ITSDaten$Herzfrequenz), 
         alternative = "less", conf.level = 0.99)$conf.int
## Kendall
KendallTauB(ITSDaten$Temperatur, ITSDaten$Herzfrequenz, conf.level = 0.98)
```
